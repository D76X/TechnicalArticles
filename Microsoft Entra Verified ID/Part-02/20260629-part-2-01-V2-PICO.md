...
![Decentralized Identifier Architecture](./02.DecentralizedIdentifierArchitecture-improved.png)  
...
[DID:Web](https://learn.microsoft.com/en-us/entra/verified-id/how-to-register-didwebsite) is a simple, permission-based method that ties a DID to an organization's existing web domain. There is no blockchain involved.
...
![SIOP v2 Flow](./03.SIOPv2-improved.png)  
...
3. **The verifier retrieves the issuer's public key.** Using the issuer's DID, the verifier resolves the DID through the Trust System (`did:web`) to retrieve the issuer's DID Document and extract the public key.
...
*Quick Setup and Advanced Setup both use DID:Web. The difference is who hosts the `did.json` file and Key Vault — Microsoft, or the organization itself — which determines whether the resulting DID carries the organization's own domain name.*
...
4. The result is returned to the verifier as a **confidence score** only. The actual photo and biometric data are never shared with or stored by the verifier — nor retained by Microsoft Authenticator, Verified ID, or Azure AI once the check completes.

The confidence score thresholds are statistically significant:

- The **minimum confidence score of 50%** corresponds to a false positive rate of 1 in 100,000
...
- **Basic issuance and verification:** Covered under Verified ID's Baseline Protection, which includes up to 50,000 transactions per month at no additional cost (a transaction being any issuance, verification, or revocation). Beyond that volume, an [Entra ID P1 or P2](https://learn.microsoft.com/en-us/entra/fundamentals/whatis#microsoft-entra-id-licensing) license is required to continue.
- **Face Check:** Requires the [Microsoft Entra Suite](https://learn.microsoft.com/en-us/entra/verified-id/using-facecheck#licensing), which includes an allowance of **8 Face Check verifications per licensed seat per month, pooled at the tenant level** across all Face Check scenarios, with additional checks billed per use beyond that allowance.
...
Because pricing and packaging change periodically, always confirm current figures against the [Microsoft Entra Verified ID pricing page](https://azure.microsoft.com/en-us/pricing/details/microsoft-entra-verified-id/) before budgeting a deployment.
...
- **The Trust System** (`did:web` in Microsoft Entra Verified ID today) makes DID Documents available to any party without central coordination.
...
The architecture is designed so that trust is rooted in mathematics and open standards, not in the continued availability of any single service or vendor. That is what makes it genuinely different from federation or centralized identity, and what makes it the right foundation for the next generation of digital trust.
