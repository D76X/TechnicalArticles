...
John Savill's videos on [quantum-safe security][1], [post-quantum cryptography][12], and [quantum computing][13] approach the subject from complementary directions: what quantum computers might change, which cryptographic systems are affected, and what organizations can do to prepare. The practical conclusion is not to wait for a machine that can break current cryptography. It is to understand what is at risk, use the new standards carefully, and begin a measured migration while there is time to test it.
...
A classical computer stores information in bits, each of which is either 0 or 1. A quantum computer uses quantum bits, or *qubits*. A qubit can be in a *superposition* of the 0 and 1 states, with mathematical amplitudes that determine the probabilities of the results when it is measured. Measurement produces an ordinary result, such as 0 or 1; it does not let us read out every possible state at once.[11][13]

Quantum algorithms use operations called gates to change the state of qubits. By carefully arranging those operations, they can make probability amplitudes interfere: some results become more likely and others less likely. *Entanglement* is a quantum relationship between qubits whose states cannot be described independently. Together, these ideas can provide an advantage for certain specially structured problems, not a general speed boost for every calculation.[11][13]

This is why "a quantum computer tries every answer at once" is a misleading shorthand. Although a multi-qubit system can represent a superposition of many states, measurement returns limited classical information. A useful algorithm must manipulate the system so the desired answer can be recovered from that measurement.[11]

Building a large, reliable quantum computer is also a substantial engineering challenge. Qubits are vulnerable to errors, and fault-tolerant computing requires detecting and correcting those errors; robust logical qubits can require many physical qubits. The potential cryptographic threat is therefore about what a sufficiently capable, fault-tolerant machine could do, not a claim that today's quantum computers can break deployed encryption.[11]

The main security concern is *public-key cryptography*. A sufficiently capable, fault-tolerant quantum computer running Shor's algorithm could break mathematical problems on which widely used systems such as RSA and elliptic-curve cryptography depend. These systems are used for tasks such as agreeing on secret keys and signing data. A successful attack could expose protected communications or let an attacker forge signatures and impersonate trusted systems.[11]
...
Quantum-safe security is a cryptographic modernization effort, not a prediction about the exact year a quantum computer will arrive. Quantum computers may eventually threaten specific public-key systems, but they do not simply make every task faster or reveal all states at once. The standards are now published, the migration will be gradual, and the systems that need the most attention are often the least visible. Start with an inventory, prioritize by data lifetime and upgrade lead time, and build the ability to change cryptography safely. That is how organizations can reduce risk now and be ready to adopt PQC across their Microsoft environments as supported capabilities mature.
...
[11] Microsoft Learn, [What is quantum computing? (Azure Quantum)](https://learn.microsoft.com/azure/quantum/overview-understanding-quantum-computing).

[12] John Savill's Technical Training, ["Post Quantum Cryptography (PQC) overview and why you should care!" (YouTube)](https://www.youtube.com/watch?v=QQMLzxqJlMA&t=6s).

[13] John Savill's Technical Training, ["Quantum Computing for Everyone" (YouTube)](https://www.youtube.com/watch?v=x5Ohhi3YTKY).
